...
def show_on_report(batch_idx, num_batches,loss_fake, loss,loss_de,loss_dcp, loss_cf,  elapsed):
    """Formats training stats."""

    clear_line()
    dec = int(np.ceil(np.log10(num_batches)))
    print('Batch {:>{dec}d} / {:d} | Avg loss: {:>1.5f}| loss_fake: {:>1.5f}|loss_de: {:>1.5f}|loss_dcp: {:>1.5f}  |loss_cf: {:>1.5f} | Avg train time / batch: {:d} ms'.format(batch_idx + 1, num_batches, loss_fake,loss, loss_de,loss_dcp, loss_cf, int(elapsed), dec=dec))
...
def to_ssim_skimage(dehaze, gt):
    ...
    ssim_list = [ski_ssim(dehaze_list_np[ind],  gt_list_np[ind], data_range=1, multichannel=True) for ind in range(len(dehaze_list))]
...
def save_image(dehaze, image_name, category):
    ...
    for ind in range(batch_num):
        ...
        # utils.save_image(dehaze_images[ind], './{}_results/{}'.format(category, image_name[ind][:-4] + 'png'))
...
def validation_PSNR(net, val_data_loader, device, category, save_tag=False):
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        ...
        with torch.no_grad():
            ...
            haze = haze.to(device)
            gt = gt.to(device)
            h, w = haze.size(2), haze.size(3)
            # haze_name = haze_name[0]

            pad_h = h % 16
            pad_w = w % 16
            haze = haze[:, :, 0:h - pad_h, 0:w - pad_w]
            gt = gt[:, :, 0:h - pad_h, 0:w - pad_w]

            # dehaze = net(haze)
            cor, trr = net.Encoder(haze)


            # print(trr.size())
            dehaze, query_der = net.Decoder( cor, trr, s=2)
            # print(query_der)

        # --- Calculate the average PSNR --- #
        psnr_list.extend(to_psnr(dehaze, gt))

        # lpips_score1 = loss_fn(dehaze, gt).item()
        # print(lpips_score1)
        image1 = dehaze
        image2 = gt

        lpips_list1.extend(loss_fn.forward(image1, image2))#.item()
    ...
    avr_ssim = sum(ssim_list) /1 #len(ssim_list)
    avr_LPIPS = sum(lpips_list1) / len(lpips_list1)
    return avr_psnr, avr_ssim,avr_LPIPS
